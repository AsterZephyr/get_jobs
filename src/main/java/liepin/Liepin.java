...
public class Liepin {
    ...
    private static String getSearchUrl() {
        return baseUrl +
                ...
                JobUtils.appendParam("workYearCode", config.getWorkYearCode()) +
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (text.contains("聊一聊")) {
                try {
                    ...
                    input.click();
                    input.sendKeys("Hello, I am interested in this job position.");
                    ...
                    // 备用选项
                    try {
                        WebElement fallbackButton = CHROME_DRIVER.findElement(By.xpath("//button[@class='fallback-button-class']"));
                        fallbackButton.click();
                    } catch (Exception fallbackException) {
                        log.error("Fallback option also failed: {}", fallbackException.getMessage());
                    }
                }
            }
